const CustomPromise = class  {
  ...
  then (onfulfilled, onrejected) {
    ...
    this.then(
      result => CustomPromise.resolved(callback()).then(() => result),
      reason => CustomPromise.resolved(callback()).then(() => { throw reason })
    )
  }

  static all (values) {

    const res = Array(values.length).fill(undefined)
    return new CustomPromise((resolve, reject) => {
      for (let index = 0; index < values.length; index++) {
        ...
        (function (arr, p, i) {
          p.then((result) => {
            arr[i] = result
            if (arr.every(item => item !== undefined)) resolve(res)
          }, (reason) => {
            reject(reason)
          })
        })(res, element, index)
